    ...
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                ...
                i = 1;
                if (req.body.scope) {
                    campos.push("scope = $".concat(i++));
                    valores.push(req.body.scope);
                }
                if (req.body.time) {
                    campos.push("time = $".concat(i++));
                    valores.push(req.body.time);
                }
                if (req.body.date) {
                    campos.push("date = $".concat(i++));
                    valores.push(req.body.date);
                }
                if (req.body.duration) {
                    campos.push("duration = $".concat(i++));
                    valores.push(req.body.duration);
                }
                if (req.body.location) {
                    campos.push("location = $".concat(i++));
                    valores.push(req.body.location);
                }
                valores.push(id);
                query = "UPDATE agendas SET ".concat(campos.join(', '), " WHERE id = $").concat(i, " RETURNING *");
